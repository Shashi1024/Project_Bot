...
dep_words = pickle.load(open('Nrm_1/DepWords.pkl','rb'))
dep_classes = pickle.load(open('Nrm_1/DepClasses.pkl','rb'))
model = load_model('Nrm_1/chatBot_model.h5')
dep_model=pickle.load(open('Nrm_1/The_Palmist_new3.pkl','rb'))
dep_eitk = load_model('Nrm_1/DepPredict_eitk.h5')
...
def bow_dep(sentence):
    sentence_words = clean_sen_dep(sentence)
    bag = [0]*len(dep_words)
    for w in sentence_words:
        for i, word in enumerate(dep_words):
            if word == w:
                bag[i] = 1
    return np.array(bag)
...
def eitk_dep(sentence):
    bow = bow_dep(sentence)
    res = dep_eitk.predict(np.array([bow]))[0]
    ERROR_THRESHOLD = 0.02
    results = [[i,r] for i,r in enumerate(res) if r> ERROR_THRESHOLD]
    results.sort(key = lambda x:x[1], reverse=True)
    return_list = []
    for r in results:
        return_list.append({'intent': dep_classes[r[0]], 'probability': str(r[1]) })
    return return_list
...
while True:
    ...
    if ints[0]['intent'] == 'symptoms':
        nn_extrct(message)
        ints_dep = eitk_dep(message)
        if ints_dep:
            ln.append(ints_dep[0]['intent'])

    res = get_res(ints, intents)
    # print("_____",ints_dep,"_____")
